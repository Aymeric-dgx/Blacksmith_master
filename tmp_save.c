...
int main(int argc, char* argv[]) {
    ...
    SDL_Rect background_stubborn_separation_rect = {0, stubborn_rect.h-4, screen.w, 4};
    ...
    SDL_Rect correcting_switch_rect = {0, forge_switch_rect.y+4, 4, forge_switch_rect.h-8}; // NB : ici on fait y+4, w=4 etc car le rect à une epaissuer de bordure de 4 pixels (cf plus bas dans le code)
    ...
    while(running) {
        ...
        print_rect(renderer, forge_switch_rect, black, 0, 4);
        print_rect(renderer, shop_switch_rect, black, 0, 4);
        print_rect(renderer, mine_switch_rect, black, 0, 4);
        print_rect(renderer, stock_switch_rect, black, 0, 4);
        print_rect(renderer, tree_switch_rect, black, 0, 4);
        for(int i=0 ; i<5 ; i++) write_text(renderer, all_switch_rect[i], switch_rect_text[i], font, black, 18);


        // Correction des doubles barres des switch_rect (et des sitch_rect des extremité pour pas faire une bande noire contre l'écran)
        for(int i=0 ; i<6 ; i++) {
            if(i==5) correcting_switch_rect.x-= 4; // Cas particulier pour la denrière, sinon on print sur le "prochain rect" (qui n'existe pas) et pas la fin du dernier
