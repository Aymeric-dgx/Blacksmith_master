...
/*
NB organisation backend/frontend

Séparer la gestion du backend (calcul temps, salaire, production, solde, clients, ...) et le frontend (affichage utilisateur)

EXEMPLE

int main(int argc, char* argv) {
    [...Initialisation SDL, structs, ...]
    ...
    while(running) {
        ...
        // Partie affichage interface utilisateur (purement graphique)
        switch (nb_window) {
            case "blacksmith": blacksmith_window(SDL_Renderer renderer, ...);
            case "shop": [...]
        }

        // Partie de calcul, mise à jour, ...
        maj_fabrication_time(...);
        maj_time(...);
        [...]
    }
}
...
typedef struct joueur {
    int balance; // Solde
    int total_salary; // = nb_forgeron * salaire_forgeron + nb_vendeur * salaire_vendeur
    int hour;
    int day;
} Player;
...
// En initialiser trois : fer, argent et or
...
typedef struct objet {
    ...
    int chance_of_buying; // Va ressembler à quelque chose comme --> chance_of_buying = (1/price) * mutliplicateur ?
} Item;
